...
pub struct SDF2DParams {
    ...
    pub time: f32,
    pub beat_proximity: f32,
}

impl GPUWrite for SDF2DParams {}
const _: () = assert!(std::mem::size_of::<SDF2DParams>() == 16);
const _: () = assert!(std::mem::offset_of!(SDF2DParams, resolution) == 0);
const _: () = assert!(std::mem::size_of::<glam::Vec2>() == 8);
const _: () = assert!(std::mem::offset_of!(SDF2DParams, time) == 8);
const _: () = assert!(std::mem::size_of::<f32>() == 4);
const _: () = assert!(std::mem::offset_of!(SDF2DParams, beat_proximity) == 12);
const _: () = assert!(std::mem::size_of::<f32>() == 4);
...
pub struct SDF2DParamsInput {
    pub resolution: glam::Vec2,
    pub time: f32,
    pub beat_proximity: f32,
}

impl GraphShaderParams for SDF2DParams {
    type Data = Self;
    type Bindings = ();
    type Input = SDF2DParamsInput;

    fn input(data: &Self::Data, _bindings: &Self::Bindings) -> Self::Input {
        Self::Input {
            resolution: data.resolution,
            time: data.time,
            beat_proximity: data.beat_proximity,
        }
    }
    ...
    fn assemble_input(input: &Self::Input, _resolver: &BindingResolver<'_>) -> Self {
        Self {
            ...
            time: input.time,
            beat_proximity: input.beat_proximity,
        }
    }
}

impl GraphBindingSet for SDF2DParamsInput {
