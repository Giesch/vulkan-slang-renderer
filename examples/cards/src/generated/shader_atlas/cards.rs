...
//! generated from slang shader: cards.shader.slang
...
#[repr(C, align(16))]
pub struct CardViewerParams {
    pub mvp: MVPMatrices,
    pub texture: BindlessHandle<Sampler2D>,
    pub _padding_0: [u8; 8],
}

impl GPUWrite for CardViewerParams {}
const _: () = assert!(std::mem::size_of::<CardViewerParams>() == 208);
const _: () = assert!(std::mem::offset_of!(CardViewerParams, mvp) == 0);
const _: () = assert!(std::mem::size_of::<MVPMatrices>() == 192);
const _: () = assert!(std::mem::offset_of!(CardViewerParams, texture) == 192);
...
pub struct Resources<'a> {
    pub params_buffer: &'a UniformBufferHandle<CardViewerParams>,
}
...
impl Shader {
    ...
    fn vert_spv(&self) -> Vec<u32> {
        let bytes = include_bytes!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/shaders/compiled/cards.vert.spv"
        ));
    ...
    fn frag_spv(&self) -> Vec<u32> {
        let bytes = include_bytes!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/shaders/compiled/cards.frag.spv"
        ));
        let byte_reader = &mut Cursor::new(bytes);
        read_spv(byte_reader).expect("failed to convert spv byte layout")
    }
