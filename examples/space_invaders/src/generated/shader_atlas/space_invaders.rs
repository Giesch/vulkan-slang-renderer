...
//! generated from slang shader: space_invaders.shader.slang
...
pub struct SpaceInvadersParams {
    ...
    pub sprite_sheet: BindlessHandle<Sampler2D>,
    pub _padding_0: [u8; 8],
}

impl GPUWrite for SpaceInvadersParams {}
const _: () = assert!(std::mem::size_of::<SpaceInvadersParams>() == 96);
...
const _: () = assert!(std::mem::offset_of!(SpaceInvadersParams, sprite_sheet) == 80);
...
impl GraphParamBindingSet for SpaceInvadersParamsBindings {
...
impl Shader {
    pub fn init() -> Self {
        let json_str = include_str!(concat!(
            ...
            "/shaders/compiled/space_invaders.json"
        ));
    ...
    fn vert_spv(&self) -> Vec<u32> {
        let bytes = include_bytes!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/shaders/compiled/space_invaders.vert.spv"
        ));
    ...
    fn frag_spv(&self) -> Vec<u32> {
        let bytes = include_bytes!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/shaders/compiled/space_invaders.frag.spv"
        ));
        let byte_reader = &mut Cursor::new(bytes);
        read_spv(byte_reader).expect("failed to convert spv byte layout")
    }
