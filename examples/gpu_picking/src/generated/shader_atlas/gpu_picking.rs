...
//! generated from slang shader: gpu_picking.shader.slang
...
pub struct GpuPickingParamsData {
    pub camera: RayMarchCamera,
    pub picked_object_id: u32,
    pub cube_count: u32,
}
...
#[derive(Debug, Clone, Copy)]
pub struct GpuPickingParamsInput {
    pub camera: RayMarchCamera,
    pub picked_object_id: u32,
    pub cube_count: u32,
    pub cubes: ReadBufferBinding<Cube>,
...
impl GraphShaderParams for GpuPickingParams {
    type Data = GpuPickingParamsData;
    type Bindings = GpuPickingParamsBindings;
    type Input = GpuPickingParamsInput;

    fn input(data: &Self::Data, bindings: &Self::Bindings) -> Self::Input {
        Self::Input {
            camera: data.camera,
            picked_object_id: data.picked_object_id,
    ...
    fn assemble_input(input: &Self::Input, resolver: &BindingResolver<'_>) -> Self {
        Self {
            camera: input.camera,
            picked_object_id: input.picked_object_id,
...
impl GraphBindingSet for GpuPickingParamsInput {
    fn visit(&self, f: &mut dyn FnMut(GraphBinding)) {
        f(GraphBinding::Buffer(self.cubes.erased()));
    }
}
...
impl Shader {
    pub fn init() -> Self {
        let json_str = include_str!(concat!(
            ...
            "/shaders/compiled/gpu_picking.json"
        ));
    ...
    fn vert_spv(&self) -> Vec<u32> {
        let bytes = include_bytes!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/shaders/compiled/gpu_picking.vert.spv"
        ));
    ...
    fn frag_spv(&self) -> Vec<u32> {
        let bytes = include_bytes!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/shaders/compiled/gpu_picking.frag.spv"
        ));
        let byte_reader = &mut Cursor::new(bytes);
        read_spv(byte_reader).expect("failed to convert spv byte layout")
    }
