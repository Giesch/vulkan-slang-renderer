...
//! generated from slang shader: dragon.shader.slang
...
impl GPUWrite for DragonParams {}
const _: () = assert!(std::mem::size_of::<DragonParams>() == 16);
const _: () = assert!(std::mem::offset_of!(DragonParams, resolution) == 0);
const _: () = assert!(std::mem::size_of::<glam::Vec2>() == 8);
const _: () = assert!(std::mem::offset_of!(DragonParams, time) == 8);
...
impl Shader {
    ...
    fn vert_spv(&self) -> Vec<u32> {
        let bytes = include_bytes!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/shaders/compiled/dragon.vert.spv"
        ));
    ...
    fn frag_spv(&self) -> Vec<u32> {
        let bytes = include_bytes!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/shaders/compiled/dragon.frag.spv"
        ));
        let byte_reader = &mut Cursor::new(bytes);
        read_spv(byte_reader).expect("failed to convert spv byte layout")
    }
