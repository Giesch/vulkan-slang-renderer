...
9. [`pipeline_config` consumes the atlas entry, so a shader can build only one pipeline](#9-pipeline_config-consumes-the-atlas-entry-so-a-shader-can-build-only-one-pipeline) — ergonomics debt, noisy call sites in `watercolor`
...
## 9. `pipeline_config` consumes the atlas entry, so a shader can build only one pipeline
