...
use mltrs_renderer::shaders::{
    ReflectedComputeShader, ReflectedShader, prepare_reflected_compute_shader,
    prepare_reflected_shader,
};
...
/// The engine slang modules shipped with the CLI, written by `mltrs shaders
/// init`. Embedded so `cargo install mltrs-cli` is self-contained.
pub const ENGINE_SLANG_MODULES: &[(&str, &str)] = &[
