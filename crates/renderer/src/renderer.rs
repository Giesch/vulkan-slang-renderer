...
///
/// Lives here rather than beside the Game trait so the renderer crate does not
/// depend on its consumer; `mltrs::game` re-exports it, so Game impls are
/// unaffected.
...
impl<'f> FrameRenderer<'f> {
    /// Constructed once per frame by the app loop, which lives in `mltrs`.
